...
export const getAllPurchase = async (req, res) => {
  ...
  try {
    if (last_id < 1) {
      result =
        await prisma.$queryRaw`SELECT p.id, p.code, p.date, p.note, p.userId, u.name FROM Purchase p INNER JOIN User u ON p.userId = u.id
        WHERE (
          code LIKE ${`%${search}%`}
          OR
          date LIKE ${`%${search}%`}
    ...
    logger.error(
      "controllers/purchase.controller.js : getAllPurchases : " + error.message
    );
    return res.status(500).json({
      message: error.message,
      result: null,
...
export const getPurchaseById = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      message: "Purchase found by id successfully",
      result: result,
    });
  } catch (error) {
    logger.error(
...
export const generatePdf = async (req, res) => {
  ...
  let html = fs.readFileSync("./src/templates/PurchaseTemplate.html", "utf8");
  let options = {
    format: "A4",
    ...
    header: {
      height: "0.1mm",
      // contents: '<div style="text-align: center;">Author: saya riky</div>',
      contents: "",
    },
    footer: {
      height: "28mm",
...
export const generateExcel = async (req, res) => {
  ...
  try {
    ...
    const data = await prisma.$queryRaw`
      SELECT o.code, o.date, o.note, o.ppn, o.grandTotal, d.productName, d.price, d.qty, (d.price * d.qty) as totalPrice FROM Purchase o 
...
export const purchaseYearly = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      message: "Purchase yearly successfully",
      result: array,
    });
  } catch (error) {
    logger.error(
