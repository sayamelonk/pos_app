...
const EditProduct = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    setName(out.data.result.productName);
    setQty(out.data.result.qty);
    setPrice(out.data.result.price);
    setCategory({
      value: out.data.result.categoryId,
      label: out.data.result.categoryName,
    });
  ...
  const loadCategory = useCallback(async () => {
    const out = await axiosInstance.get("api/categories", {});
        ...
        <Row className="mt-3 bg-body-tertiary rounded p-3">
          <Col>
            <form onSubmit={handleSubmit}>
              ...
              <Col sm="10">
                <Form.Control plaintext readOnly defaultValue={product.code} />
              </Col>
              <Form.Group as={Row} className="mb-3">
                <Form.Label column sm="2">
                  Category
